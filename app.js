...
function updateActiveMenuItem() {
	...
	for (var i = 0; i < pageSections.length; i++) {
		...
		} else if (i === pageSections.length - 1 && getTopOffset(pageSections[i]) < scrollPos) {
			//Check for last item
			menuItems[i].classList.add('active-link');
		} else if (getTopOffset(pageSections[i]) < scrollPos && getTopOffset(pageSections[i + 1]) > scrollPos) {
